...
const DEFAULT_PACKAGES = [
  { amount: 100,   min: 1.17, max: 1.80, days: 175 },
  { amount: 250,   min: 1.35, max: 1.98, days: 182 },
  { amount: 500,   min: 1.53, max: 2.16, days: 190 },
  { amount: 1000,  min: 1.71, max: 2.34, days: 197 },
  { amount: 2000,  min: 1.89, max: 2.52, days: 205 },
  { amount: 5000,  min: 2.07, max: 2.70, days: 215 },
  { amount: 10000, min: 2.16, max: 2.79, days: 230 },
  { amount: 25000, min: 2.70, max: 4.50, days: 270 },
];
...
async function getUserWithPackages(userId) {
  ...
  // ---- split earnings: own packages vs referral share ----
  // Today's referral income (profit share credited today)
  const refToday = await pool.query(
    `SELECT COALESCE(SUM(commission),0) AS s
       FROM referral_earnings
      WHERE referrer_id = $1 AND created_at::date = NOW()::date`,
    [userId]
  );
  // Today's own profit = sum of daily_profit on packages that are still active
  const ownToday = shaped
    .filter(p => p.active)
    .reduce((a, p) => a + Number(p.dailyProfit || 0), 0);

  const totalReferral = Number(user.referral_earnings || 0);
  // total_profit on the user row accumulates package profit only
  const totalOwn = Number(user.total_profit || 0);
  ...
  return {
    ...
    earnings: {
      todayOwn: Number(ownToday.toFixed(2)),
      todayReferral: Number(Number(refToday.rows[0].s || 0).toFixed(2)),
      todayTotal: Number((ownToday + Number(refToday.rows[0].s || 0)).toFixed(2)),
      totalOwn: Number(totalOwn.toFixed(2)),
      totalReferral: Number(totalReferral.toFixed(2)),
      grandTotal: Number((totalOwn + totalReferral).toFixed(2)),
    },
    points: {
      total: totalPoints,
...
async function runPayout({ position = null, force = false, mode = 'auto' } = {}) {
  ...
  try {
    ...
    // cache: userId -> referrer's user id (or null). Avoids a lookup per package.
    const referrerCache = {};
    async function referrerOf(userId) {
      if (referrerCache[userId] !== undefined) return referrerCache[userId];
      const u = await client.query('SELECT referred_by FROM users WHERE id = $1', [userId]);
      const code = u.rows.length ? u.rows[0].referred_by : null;
      let refId = null;
      if (code) {
        const rr = await client.query(
          'SELECT id FROM users WHERE referral_code = $1 AND suspended IS NOT TRUE',
          [code]
        );
        if (rr.rows.length && rr.rows[0].id !== userId) refId = rr.rows[0].id;
      }
      referrerCache[userId] = refId;
      return refId;
    }
    ...
    for (const pkg of pkgs.rows) {
      ...
      // ---- referrer's ongoing share of this profit ----
      // Paid by the company, not deducted from the invitee. Test packages excluded.
      if (!pkg.is_test && REFERRAL_PROFIT_SHARE > 0) {
        const refId = await referrerOf(pkg.user_id);
        if (refId) {
          const share = profit * REFERRAL_PROFIT_SHARE;
          if (share > 0) {
            await client.query(
              `UPDATE users
                  SET balance = balance + $1, referral_earnings = referral_earnings + $1
                WHERE id = $2`,
              [share, refId]
            );
            await client.query(
              `INSERT INTO referral_earnings (id, referrer_id, referred_id, deposit_amount, commission)
               VALUES ($1,$2,$3,$4,$5)`,
              [makeId('RP'), refId, pkg.user_id, 0, share]
            );
            referralPaid += share;
          }
        }
    ...
    console.log(`[payout ${mode}] ${date}: paid ${paidCount} package(s), $${totalPaid.toFixed(2)}, referral $${referralPaid.toFixed(2)}, completed ${completed}`);
    ...
    return {
      ...
      referralPaid: Number(referralPaid.toFixed(2)),
      breakdown,
      mode,
    };
  } catch (err) {
...
// ---- My transaction history (user) ----
// Everything that has moved this user's balance: deposits (packages activating),
// daily profit, referral commission, and their own withdrawals.
app.get('/api/my/transactions', auth, async (req, res) => {
  try {
    ...
    // packages purchased (deposits)
    const p = await pool.query(
      `SELECT amount, created_at, is_test FROM packages
        WHERE user_id = $1 ORDER BY created_at DESC LIMIT 100`,
      [req.userId]
    );
    p.rows.forEach(r => out.push({
      type: 'deposit',
      amount: Number(r.amount),
      status: r.is_test ? 'test' : 'confirmed',
      date: r.created_at,
    }));

    // referral commissions earned
    const re = await pool.query(
      `SELECT commission, created_at FROM referral_earnings
        WHERE referrer_id = $1 ORDER BY created_at DESC LIMIT 100`,
      [req.userId]
    );
    re.rows.forEach(r => out.push({
      type: 'commission',
    ...
    // summary of withdrawals specifically
    const wSummary = {
      totalWithdrawn: w.rows.filter(r => r.status === 'approved')
        .reduce((a, r) => a + Number(r.amount), 0),
      pending: w.rows.filter(r => r.status === 'pending')
        .reduce((a, r) => a + Number(r.amount), 0),
      count: w.rows.length,
    };

    res.json({ transactions: out.slice(0, 60), withdrawals: wSummary });
  } catch (e) {
